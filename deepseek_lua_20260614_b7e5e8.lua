-- Dojo Hub | Auto Level - Bản fix mượt mà
-- Không giật, tween qua quái khác khi chết
...
local function EquipMelee()
    ...
    local priority = {"Godhuman", "Dragon Talon", "Electric Claw", "Sharkman Karate", "Death Step", "Superhuman"}
...
local function TweenToPosition(pos)
    ...
    local duration = math.max(0.2, dist / _G.TweenSpeed)
    
    currentTween = TweenService:Create(hrp, TweenInfo.new(duration, Enum.EasingStyle.Linear), {CFrame = pos})
    currentTween:Play()
...
local function StartSpin(target)
    if spinTarget == target and isSpinning then return end
    ...
    spinConnection = RunService.RenderStepped:Connect(function()
        if not _G.AutoLevel or not spinTarget or not spinTarget.Parent then
            StopSpin()
            return
        end
        ...
        local mhrp = spinTarget:FindFirstChild("HumanoidRootPart")
        if not mhrp then return end
        
        local hum = spinTarget:FindFirstChild("Humanoid")
        ...
        -- Xoay chậm và mượt
        spinAngle = spinAngle + (_G.SpinSpeed * 0.016)
        ...
        -- Chỉ cập nhật CFrame, không tween để tránh giật
...
task.spawn(function()
    while task.wait() do
        ...
        pcall(function()
            ...
            -- Tìm quái
            local target, dist = FindBestTarget()
            ...
            if target then
                ...
                if mhrp and mhum and mhum.Health > 0 then
                    ...
                    -- Nếu xa quá thì tween đến
                    if dist > _G.SpinRadius + 5 then
                        StopSpin()
                        local newY = mhrp.Position.Y + _G.MobHeight
                        TweenToPosition(CFrame.new(mhrp.Position.X, newY, mhrp.Position.Z))
                        task.wait(0.3)
                    else
                        -- Bắt đầu xoay vòng
                        StartSpin(target)
                ...
                -- Không có quái, tween về spawn point
                StopSpin()
                TweenToPosition(CFrameMon)
                task.wait(0.5)
            end
        end)
    end
...
task.spawn(function()
    while task.wait(1) do
        if _G.AutoLevel then
            local lvl = plr.Data.Level.Value
            Info:SetDesc(string.format("Level: %d | World: %d\nQuái: %s\nĐang xoay quanh: %s", lvl, _G.World, Mon, spinTarget and "✅" or "⏳"))
        else
...
print("Dojo Hub - Đã tải! Bật Auto Level để bắt đầu")