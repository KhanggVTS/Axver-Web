-- Dojo Hub | Auto Level + Fast Attack (Complete)
-- Dựa trên cơ chế từ AxverHub
...
local function GetQuestData()
    ...
    elseif _G.World == 3 then
        ...
        elseif lvl <= 2599 then
            ...
            data = {Mon = "Grand Devotee", Quest = "SubmergedQuest3", Level = 2, QuestPos = CFrame.new(9636,-1992,9609), MobPos = CFrame.new(9557,-1928,9859)}
        end
    end
    return data
end
...
task.spawn(function()
    while task.wait() do
        ...
        if mob then
            ...
            -- Di chuyển đến quái nếu ở xa
            if dist > 15 then
                TweenPlayer(mob.HumanoidRootPart.CFrame * CFrame.new(0, _G.MobHeight, 0))
                task.wait(0.1)
            end
    ...
    while task.wait(1) do
        if _G.AutoLevel then
            ...
            infoParagraph:SetDesc(string.format(
                "Level: %d\nWorld: %d\nQuái: %s\nFast Attack: %s\nBring Mob: %s",
                lvl, _G.World, questData.Mon or "N/A",
...
print("Tác giả: Dojo - Dựa trên cơ chế AxverHub")