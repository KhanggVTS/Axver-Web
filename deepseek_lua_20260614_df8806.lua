-- Dojo Hub | Auto Level + Fast Attack (Xoay vòng trên đầu quái)
-- Fast Attack từ file bạn gửi + Auto Level từ AxverHub
-- Xoay vòng tròn liên tục trên đầu quái + Auto trang bị Melee

local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield'))()

local Window = Rayfield:CreateWindow({
    Name = "Dojo Hub | Auto Level + Fast Attack",
    LoadingTitle = "Chờ Đợi",
...
_G.BringRange = 235
_G.MaxBringMobs = 2
_G.SpinRadius = 15  -- Bán kính xoay vòng
_G.SpinSpeed = 30   -- Tốc độ xoay (độ/frame)
_G.World = nil
_G.SelectedWeapon = "Melee"  -- Vũ khí mặc định
...
local function EquipMelee()
    ...
    -- Danh sách các loại vũ khí Melee/Fighting Style
    local meleeTypes = {"Melee", "Fighting Style"}
    local meleeTools = {}
    
    -- Tìm kiếm trong Backpack
    for _, tool in pairs(plr.Backpack:GetChildren()) do
        if tool:IsA("Tool") then
            for _, mType in pairs(meleeTypes) do
                if tool.ToolTip == mType then
    ...
    -- Ưu tiên các fighting style mạnh
    local priorityOrder = {"Godhuman", "Dragon Talon", "Electric Claw", "Sharkman Karate", "Death Step", "Superhuman", "Fishman Karate", "Electro", "Black Leg", "Combat"}
    
    local selectedTool = nil
    
    -- Tìm theo thứ tự ưu tiên
    for _, priority in pairs(priorityOrder) do
        for _, tool in pairs(meleeTools) do
            if tool.Name == priority then
                selectedTool = tool
                break
            end
        end
        if selectedTool then break end
    end
    
    -- Nếu không có trong danh sách ưu tiên, lấy cái đầu tiên
    if not selectedTool and #meleeTools > 0 then
        selectedTool = meleeTools[1]
    end
    
    -- Trang bị
    if selectedTool and not char:FindFirstChild(selectedTool.Name) then
...
-- Kiểm tra đã có vũ khí Melee chưa
local function HasMeleeEquipped()
    ...
    for _, tool in pairs(char:GetChildren()) do
        if tool:IsA("Tool") then
            if tool.ToolTip == "Melee" or tool.ToolTip == "Fighting Style" then
                return true
            end
        end
    end
...
local function CheckQuest()
    ...
    elseif _G.World == 3 then
        ...
        elseif lvl<=2574 then
            ...
            CFrameQuest=CFrame.new(-16665,105,1580); CFrameMon=CFrame.new(-16542.4824,146.675156,1529.61401)
        elseif lvl<=2599 then
            Mon="Skull Slayer"; LevelQuest=2; NameQuest="TikiQuest3"; NameMon="Skull Slayer"
            CFrameQuest=CFrame.new(-16665,105,1580); CFrameMon=CFrame.new(-16849.9336,147.005066,1640.88354)
        else
...
-- ============ XOAY VÒNG TRÒN TRÊN ĐẦU QUÁI ============
local currentAngle = 0
local currentMob = nil
local isSpinning = false

local function UpdateSpinPosition()
    ...
    local offsetX = math.cos(radian) * _G.SpinRadius
    local offsetZ = math.sin(radian) * _G.SpinRadius
    
    -- Vị trí mới: Xoay quanh quái, giữ độ cao cố định
    local newX = mobHrp.Position.X + offsetX
    local newZ = mobHrp.Position.Z + offsetZ
    local newY = mobHrp.Position.Y + _G.MobHeight
...
local function BringEnemy()
    ...
    task.spawn(function()
        ...
        for _, mob in pairs(Workspace.Enemies:GetChildren()) do
            ...
            if hum and root and hum.Health > 0 and mob ~= currentMob then
                local distToTarget = (root.Position - mobPos).Magnitude
                local distToPlayer = (root.Position - hrp.Position).Magnitude
                
                if distToTarget <= _G.BringRange and distToPlayer > 10 then
                    -- Tránh kéo boss
                    local isBoss = false
                    local bossList = {"Darkbeard", "Greybeard", "Saber Expert", "Cake Prince", "Dough King", "rip_indra"}
                    ...
                    if not isBoss then
                        ...
                        pcall(function()
                            ...
                            local destCF = CFrame.new(hrp.Position.X, root.Position.Y, hrp.Position.Z)
...
local function HasM1Fruit()
    ...
    if tool and tool.ToolTip == "Blox Fruit" then
        ...
        local m1Fruits = {"kitsune", "t-rex", "trex", "pain", "control", "dragon", "buddha", "leopard"}
        for _, name in pairs(m1Fruits) do
            if string.find(fruitName, name) then
                return true
            end
        end
    end
...
local function PerformAttack(mob)
    ...
    for i = 1, _G.Combo do
        hit:FireServer(hrp, {{mob, hrp}}, nil, nil, tostring(plr.UserId):sub(2,4))
    end
    
    local weapon = Combat:GetWeaponName(tool)
    Combat:ApplyDamageHighlight(mob, char, weapon, hrp)
    
    -- Fast Attack cho Fruit
    if HasM1Fruit() then
        task.spawn(FastAttackFruit)
    end
end
...
local function FindNearestMob()
    ...
    for _, mob in pairs(Workspace.Enemies:GetChildren()) do
        ...
        if mobHrp and mobHum and mobHum.Health > 0 and mob.Name == Mon then
...
task.spawn(function()
    while task.wait() do
        ...
        -- AUTO TRANG BỊ MELEE MỖI KHI KHÔNG CÓ
        ...
        if not hasQuest or not string.find(questTitle, NameMon or "") then
            AcceptQuest()
            task.wait(0.5)
            continue
        end
...
-- ============ UI ============
Tab:CreateToggle({
    Name = "🔥 Bật Auto Level + Fast Attack",
    CurrentValue = false,
    Callback = function(v)
        _G.AutoLevel = v
        if v then
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Dojo Hub",
                Text = "Đã bật Auto Level + Fast Attack!",
                Duration = 2
            })
        else
            currentMob = nil
...
Tab:CreateToggle({
    Name = "🔄 Bật Bring Mob (1-2 con/lần)",
    CurrentValue = true,
    Callback = function(v)
        _G.BringMob = v
    end
...
Tab:CreateButton({
    Name = "🔄 Trang Bị Melee Ngay",
    Callback = function()
        EquipMelee()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Dojo Hub",
            Text = "Đã trang bị Melee!",
            Duration = 2
        })
    end
})
...
task.spawn(function()
    while task.wait(1) do
        if _G.AutoLevel then
            local lvl = plr.Data.Level.Value
            local hasMelee = HasMeleeEquipped()
            infoParagraph:SetDesc(string.format(
                "Level: %d | World: %d\nQuái: %s\nĐộ cao: %d | Bán kính: %d\nMelee: %s | Kéo: %d con\nXoay vòng: %.1f°/s",
                lvl, _G.World, Mon or "N/A",
                _G.MobHeight, _G.SpinRadius,
                hasMelee and "✅ Đã trang bị" or "❌ Chưa có",
                _G.MaxBringMobs,
                _G.SpinSpeed * 60
            ))
        else
...
print("Dojo Hub | Auto Level + Fast Attack - Đã tải thành công!")
print("Tính năng: Xoay vòng tròn trên đầu quái + Auto trang bị Melee")